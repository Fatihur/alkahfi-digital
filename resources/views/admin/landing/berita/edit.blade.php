...
@section('content')
    <div class="page-header">
        <div>
            ...
            <p class="page-subtitle">Edit berita atau artikel.</p>
        </div>
    </div>

    <div class="card">
        <div class="card-body">
            <form action="{{ route('admin.landing.berita.update', $berita) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="row">
                    ...
                    <div class="col-6">
                        <div class="form-group">
                            ...
                            @if($berita->gambar)
                                <div class="mb-2">
                                    <img src="{{ Storage::url($berita->gambar) }}" alt="" style="max-height: 100px; border-radius: 4px;">
                                </div>
                            @endif
                ...
                <div class="form-group">
                    <label class="form-check form-switch">
                        <input type="checkbox" name="is_published" class="form-check-input" value="1" {{ old('is_published', $berita->is_published) ? 'checked' : '' }}>
                        <span>Dipublikasikan</span>
                    </label>
                </div>

                <p class="text-muted" style="font-size: 0.875rem;">
                    <i class="bi bi-eye"></i> {{ $berita->views }} views &bull;
                    <i class="bi bi-calendar"></i> Dibuat: {{ $berita->created_at->format('d/m/Y H:i') }}
                    @if($berita->tanggal_publikasi)
                        &bull; <i class="bi bi-globe"></i> Dipublikasikan: {{ $berita->tanggal_publikasi->format('d/m/Y H:i') }}
                    @endif
                </p>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Track changes
        let originalContent = document.getElementById('konten').value;
