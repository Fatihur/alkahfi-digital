    ...
    <div class="card">
        <div class="card-header">
            <form action="" method="GET" class="d-flex gap-2">
                ...
                <select name="metode" class="form-control form-select" style="width: 120px;">
                    <option value="">Metode</option>
                    <option value="tunai" {{ request('metode') == 'tunai' ? 'selected' : '' }}>Tunai</option>
                    <option value="transfer" {{ request('metode') == 'transfer' ? 'selected' : '' }}>Transfer</option>
                    <option value="payment_gateway" {{ request('metode') == 'payment_gateway' ? 'selected' : '' }}>Online</option>
                </select>
                ...
                <tbody>
                    @forelse($pembayaran as $p)
                        <tr>
                            ...
                            <td>{{ $p->santri->nama_lengkap }}<br><small class="text-muted">{{ $p->santri->nis }}</small></td>
                            ...
                            <td>
                                @switch($p->status)
                                    ...
                                    @case('expired') <span class="badge badge-secondary">Expired</span> @break
                                @endswitch
                            </td>
                            <td>
                                <div class="d-flex gap-1">
                                    <a href="{{ route('admin.pembayaran.show', $p) }}" class="btn btn-sm btn-secondary"><i class="bi bi-eye"></i></a>
                                    @if($p->status == 'berhasil')
                                        <a href="{{ route('admin.pembayaran.cetak', $p) }}" class="btn btn-sm btn-primary" target="_blank"><i class="bi bi-printer"></i></a>
                                    @endif
                                </div>
                            </td>
                        </tr>
